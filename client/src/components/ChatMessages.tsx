    ...
    return (
      <div className="flex gap-4 py-6 px-6 transition-colors">
        ...
        <div className="flex-1 space-y-2 overflow-hidden">
          ...
          <div className="text-zinc-100 whitespace-pre-wrap wrap-break-word leading-7">{message.payload.text}</div>
        </div>
      </div>
    );
      ...
      <div className="flex gap-4 py-4 px-6">
        ...
        <div className="flex-1 space-y-2">
          <div className="text-sm text-zinc-400 italic">
            Using tool: <span className="text-purple-400 font-medium">{message.payload.name}</span>
          </div>
          <div className="text-xs text-zinc-300 bg-purple-900/15 rounded-lg p-3 font-mono whitespace-pre-wrap">{JSON.stringify(message.payload.args, null, 2)}</div>
        </div>
      </div>
    );
      ...
      <div className="flex gap-4 py-4 px-6">
        ...
        <div className="flex-1 space-y-2">
          <div className="text-sm text-zinc-400">
            Tool result: <span className="text-green-400 font-medium">{message.payload.name}</span>
          </div>
          <div className="text-xs text-zinc-300 bg-green-900/20 rounded-lg p-3 font-mono whitespace-pre-wrap">{JSON.stringify(message.payload.result, null, 2)}</div>
